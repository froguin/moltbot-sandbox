...
- **Bootstrap Failure Visibility**: `/api/status` now starts a non-blocking bootstrap probe after kickstart and logs trimmed `start-openclaw.sh` stdout/stderr when port 18789 does not come up within 7s. This is for production troubleshooting without enabling `/debug/*`.
