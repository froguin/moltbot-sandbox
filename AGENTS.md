...
- **Sandbox Rollout Rollback**: The temporary `moltbot-v2` sandbox instance migration caused repeated container-capacity errors in production. Runtime was reverted back to the original sandbox key (`moltbot`) and recovery is handled via runtime gateway launch fallback instead of instance migration.
- **Control UI Origin Requirement**: Newer OpenClaw gateway requires either `gateway.controlUi.allowedOrigins` or `gateway.controlUi.dangerouslyAllowHostHeaderOriginFallback=true`.
...
- **Stuck Gateway Cleanup**: Runtime launch wrapper now runs `pkill -f "openclaw gateway"` before fallback launch to clear stale/hung gateway processes that block port 18789 readiness.
